...
import { CreateWaterResourceService } from "../../services/water-resource/CreateWaterResourceService";
...
class CreateWaterResourceController {
  async handle(req: Request, res: Response) {
    ...
    const propriedade_poco_amazonas = String(
      req.body.propriedade_poco_amazonas
    );
    ...
    const createWaterResourceService = new CreateWaterResourceService();

    const waterResource = await createWaterResourceService.execute({
      propriedade_id,
      tipo_cisterna,
      capacidade_cisterna,
...
export { CreateWaterResourceController };
