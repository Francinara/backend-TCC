export const SELECT_PROPERTIES_WITH_PRODUCTIVE_ACTIVITY_QUERY = `SELECT 
    p.id, 
...
export const SELECT_PROPERTIES_WITH_SELECTED_PRODUCTIVE_ACTIVITIES_QUERY = `
  AND EXISTS (
    SELECT 1
      FROM $1 $2
      WHERE p.id = $3.propriedade_id
...
export const SELECT_PRODUCTIVE_ACTIVITY_BY_PROPERTY_QUERY = `SELECT 
		* 
	FROM $1 
	WHERE propriedade_id = $2`;

export const SELECT_PRODUCTIVE_ACTIVITY_BY_ACTIVITY_QUERY = `SELECT 
...
export const SELECT_PROPERTIES_WITH_MULTIPLE_ACTIVITIES_BY_PRODUCTIVE_ACTIVITY_QUERY = `SELECT
...
export const SELECT_PROPERTY_BY_PRODUCTIVE_ACTIVITY_BY_ACTIVITY_QUERY = `SELECT 
		p.id AS propriedade_id,
    p.nome_propriedade,
    ST_AsGeoJSON(p.geom) AS geojson,
    ST_X(p.geom) AS lng, 
...
export const PRODUCTIVE_ACTIVITY_COUNT_SUMMARY_QUERY = `SELECT 
...
export const PRODUCTIVE_ACTIVITY_COUNT_SUMMARY_BY_ACTIVITY_QUERY = `SELECT DISTINCT 
		$1,
		COUNT(*)::int 
	FROM $2 
	GROUP BY $3`;

export const F = `SELECT 
    p.id AS propriedade_id,
    p.nome_propriedade,
