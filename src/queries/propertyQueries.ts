export const SELECT_PROPERTY_QUERY = `SELECT 
  p.id, 
...
export const SELECT_PROPERTY_BY_ID_QUERY = `SELECT 
  id, 
...
export const SELECT_PROPERTY_BY_AREA_QUERY = `SELECT 
    id, 
    ST_AsGeoJSON(geom) AS geojson,
    ST_X(geom) AS lng, 
    ST_Y(geom) AS lat,
...
export const SELECT_PROPERTY_BY_DISTRICT_QUERY = `SELECT
    p.id, 
    ST_AsGeoJSON(p.geom) AS geojson,
    ST_X(p.geom) AS lng, 
    ST_Y(p.geom) AS lat,
...
export const PROPERTY_COUNT_QUERY = `SELECT
  COUNT(*)::integer AS total_properties
  FROM propriedades`;

export const PROPERTY_COUNT_BY_AREA_QUERY = `SELECT 
  COUNT(*)::integer AS total_properties
  FROM propriedades
  WHERE ST_DWithin(
    geom::geography, 
...
export const PROPERTY_COUNT_BY_DISTRICT_QUERY = `SELECT
    d.id,
    d."NM_DIST",
